...
>
> ⚠️ **게이트웨이를 새로 만들지 않습니다.** Lab 4에서 만든 `vgw-hub-krc-prod-userNN`에 **연결(Connection)을 하나 더 추가**하는 작업입니다.

```
vgw-hub-krc-prod-userNN  (Lab 4에서 만든 게이트웨이 — 그대로 사용)
   ├─ 연결 ①  conn-to-idc-userNN      → lng-onprem-idc   (선택 사항 · Lab 4 Task 2)
   └─ 연결 ②  conn-azure-to-aws-userNN → lng-aws-vpc-userNN     ← 지금 추가하는 것
```

## 3-1. LNG 생성 — AWS를 "상대편"으로 등록

1. 포털에서 "로컬 네트워크 게이트웨이" 검색 → [만들기].
2. 리소스 그룹 `rg-network-prod-userNN`, 이름 `lng-aws-vpc-userNN`, 지역 `Korea Central` 입력.
3. 엔드포인트 **IP 주소** → **③번 값(AWS 터널 1 외부 IP)** 입력.
4. 주소 공간에 **`172.31.0.0/16`**(AWS VPC 대역) 입력.
...
## 3-2. Lab 4 게이트웨이에 연결 추가

1. 포털에서 **`vgw-hub-krc-prod-userNN`** 로 이동합니다 (Lab 4에서 만든 것).
2. 왼쪽 메뉴 [연결] 클릭 → [추가] 클릭.
3. 이름 `conn-azure-to-aws-userNN`, 연결 형식 **사이트 간(IPsec)** 선택.
4. 가상 네트워크 게이트웨이가 `vgw-hub-krc-prod-userNN`로 고정된 것을 확인합니다.
5. 로컬 네트워크 게이트웨이에서 `lng-aws-vpc-userNN` 선택.
...
- [ ] Azure 연결 상태가 **"연결됨"** 인가
- [ ] AWS 터널 상태가 **UP** 인가
- [ ] 게이트웨이에 AWS 연결(`conn-azure-to-aws-userNN`)이 있는가
