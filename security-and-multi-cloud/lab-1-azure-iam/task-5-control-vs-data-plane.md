# Task 5 - 관리 평면 vs 데이터 평면 (Reader의 함정)
...
<tr>
<td>구분</td>
<td>관리 평면 (Control Plane)</td>
<td>데이터 평면 (Data Plane)</td>
</tr>
<tr>
<td>무엇을 보나</td>
<td>스토리지 계정이 존재한다 / 설정이 어떻다 — 여기까지만</td>
<td>Blob 파일 내용, Key Vault 비밀 값 등 '안에 든 것'</td>
</tr>
<tr>
<td>해당 역할</td>
<td>Reader, Contributor, Owner</td>
<td>Storage Blob Data Reader / Contributor 등 별도 역할</td>
</tr>
</table>
...
3. `demo` 컨테이너에 아무 텍스트 파일 하나를 업로드합니다.
...
4. 화면 상단의 **인증 방법**이 `Microsoft Entra ID` 로 되어 있는지 확인합니다 (액세스 키로 전환하면 RBAC을 우회해 버려 실습 의미가 없어집니다).

> ❗ 여기서 "권한 줬는데 왜 안 되지?" 라는 실무 질문이 나옵니다. **Reader는 관리 평면 역할**이라 컨테이너 안의 데이터에는 닿지 못합니다.

## 5-3. 데이터 평면 역할 부여 후 재확인

1. 관리자 계정으로 돌아와 `stiamuser**` → [액세스 제어(IAM)] → [역할 할당 추가].
2. **Storage Blob Data Reader** 역할을 `sec-view-user**` 그룹에 할당합니다 (범위: 이 스토리지 계정).
3. 다시 `view-user**` 으로 로그인 → `demo` 컨테이너의 파일 목록이 **보이고**, 파일 내용도 열립니다.

> ⚠️ **반영에 몇 분 걸릴 수 있습니다.** 바로 안 되면 로그아웃 후 재로그인하세요.
>
...
- [ ] `Storage Blob Data Reader` 부여 후 파일이 열렸는가
