...
| 위치                        | 체크박스 문구                                                    | 설정             |
| ------------------------- | ---------------------------------------------------------- | -------------- |
| ① 허브 (peer-hub-to-spoke)  | 'vnet-hub…' 게이트웨이 또는 경로 서버가 트래픽을 'vnet-spoke…'에게 전달하도록 허용  | 체크             |
| ② 스포크 (peer-spoke-to-hub) | 'vnet-spoke…'이(가) 'vnet-hub…'의 원격 게이트웨이 또는 경로 서버를 사용하도록 설정 | 체크             |
| 나머지 두 개                   | 액세스 허용 · 전달된 트래픽 수신 허용                                     | Lab 3에서 이미 체크됨 |
